...
//
//   let nFTEventJSON = try? newJSONDecoder().decode(NFTEventJSON.self, from: jsonData)

import Foundation
import Alamofire

// MARK: - NFTEventJSON
struct NFTEventJSON: Codable {
    let code: Int?
    let msg: [Msg]?
}
...
struct NftEventPram: Encodable {
    let asset_contract_address, account_address: String
    let token_id: Int
    let limit, only_opensea, event_type, auction_type: String
}

func RequestNftEvent(NftParm: NftEventPram) -> NFTEventJSON{
    var Msg: NFTEventJSON?
    let queue = DispatchQueue.global()
    let sema = DispatchSemaphore(value: 0)
    print("开始upload")
    AF.request("http://10.112.110.230:8888/api/getEvents",
               method: .post,
               parameters: NftParm,
               requestModifier: { $0.timeoutInterval = 180})
        .responseDecodable(queue: queue){ (response: AFDataResponse<NFTEventJSON>) in
            debugPrint(response)
